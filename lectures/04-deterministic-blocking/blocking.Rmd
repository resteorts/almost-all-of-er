...
- ER is only performed within each block. 
...
What other evaluation metrics should we consider for entity resolution/blocking? 
...
For blocking, its paramount that our recall be as close a possible to 1. 

Example: What does it mean if we have a blocking criterion where our recall is 0.5? 

Answer: This means that we are splitting entities across blocks 50 percent of the time, which implies that then applying ER post-blocking is quite useless (as we cannot fix this error). 
