...
Let $$\gamma = (\gamma_1, \gamma_2, \ldots, \gamma_{(N_1 \times N_2)})$$
...
Fellegi and Sunter (1969) proposed two methods in their paper. 
...
The second method applies to all the comparison vectors $\gamma_i = (\gamma_i^{(1)}, \gamma_i^{(2)}, \dots, \gamma_i^{(k)})$ under the following assumptions: 

\begin{itemize}
        \item $\gamma_i \in \{0,1\}$ is a binary comparison
        \item $\{\gamma_i\}_{i=1}^k$ is conditionally independent given the true match or non-match status of the pair of records.
\end{itemize}
...
## Method 2
  ...
  Abusing notation, the idea is to consider the equations:
    ...
    \begin{itemize}
        \item Here $M$ is the set of \textbf{true} match among record pairs, $U$ is the set of true non-matches.
        \item Nowadays the EM algorithm is used.
    \end{itemize}
