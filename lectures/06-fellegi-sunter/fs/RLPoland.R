...
fileDir <- "../datasets/"
file.name <- "fullDataPoland.csv"
filesDf <- read.csv(paste0(fileDir,file.name), na.strings=c("NA"), 
                     stringsAsFactors = FALSE, colClasses = "character")

# ------------------------------- Configuration ------------------------------ #
linkingFields    <- c("gender", "yb", "province", "db", "mb", "educ")
#strLinkingFields <- c("gender", "yb")
strLinkingFields <- numeric(0)
blockPasses      <- list(c("gender"), c("yb"))
recIdField       <- "rec_id"
entIdField       <- "ent_id"
strDist          <- "levenshtein"
strCutoff        <- 0.70
