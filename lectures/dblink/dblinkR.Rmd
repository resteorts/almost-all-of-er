...
library(devtools)
library(dblinkR)
install_github("cleanzr/dblinkR") # assumes that you have dblink 
# and any required packages installed locally (java, sbt, etc)
```
...
We will run `dblink` on a local instance of Spark with 2 
cores. This is sufficient for testing purposes; for larger data sets, 
...
```{r spark-connect, message=FALSE, cache=TRUE}
sc <- spark_connect(master = "local[2]", spark_home = "/opt/spark-2.4.5-bin-hadoop2.7")
...
`dblink` requires a location on disk to save diagnostics, 
posterior samples, and the state of the Markov chain. 
...
# ATTN : Getting an error here but not from the console. 
...
- the date-related attributes as categorical (with a 
...
The Beta prior on the distortion probabilities favors low distortion.
...
# Specify Size of Latent Population 

```{r set-population-size}
# Set the latent population to the total number of records 
populationSize <- NULL 
```
...
result <- runInference(state, projectPath, sampleSize = 100,
                       burninInterval = 0, thinningInterval = 1)
```

# Add in diagnostics and evaluation metrics later. 
